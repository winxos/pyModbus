...
def ModbusCheckCalc(n: bytes, sz):
    ...
    for i in range(sz):
        tmp = n[i] ^ crc
        crc >>= 8
        crc ^= modbus_crc_table[tmp & 0xFF]
    return crc


class ModbusMaster(Thread):
    class State(Enum):
        IDLE = auto()
        SENDING = auto()
        TIMEOUT = auto()
    ...
    @staticmethod
    def __build_frame(addr, cmd, p1, p2):
        _buf = bytearray(8)
        _buf[0] = addr
        _buf[1] = cmd
        _buf[2] = p1 // 256
    ...
    def receive(self, n: bytes) -> None:
        ...
        if self.state == self.State.SENDING:
            if len(n) < 4:
                return
            if n[0] == self.sending_addr and n[1] == self.sending_cmd:
                if self.success_cb:  # if defined
                    self.success_cb(n)  # success callback
                self.state = self.State.IDLE
            else:
                self.fail_cb(1)
    ...
    def run(self) -> None:
        while True:
            if self.state == self.State.TIMEOUT:
                if self.fail_cb:  # if defined
                    self.fail_cb(0)  # fail callback
                self.state = self.State.IDLE
            elif self.state == self.State.IDLE:
                if not self._tx_q.empty():
                    data, self.success_cb, self.fail_cb = self._tx_q.get()  # fetch data and current callback
                    self.sending_addr = data[0]
                    self.sending_cmd = data[1]
                    if self.send:
                        self.send(data)
                    self.timeout_ticks = 20  # timeout 20 ticks
                    self.state = self.State.SENDING
            elif self.state == self.State.SENDING:
...
def test():
    x = ModbusMaster()
    x.set_sender(sender)
    x.read_registers(1, 3, 5, success, fail)
    sleep(0.025)
    x.receive(bytearray([1, 3, 10, 22, 13, 15]))
